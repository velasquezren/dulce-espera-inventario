  ...
  return (
    <aside className="hidden md:flex flex-col w-64 bg-white border-r border-[#e2e8f0] min-h-screen">
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-semibold tracking-wide transition-all duration-150 text-left cursor-pointer ${
                isActive
      ...
      <div className="p-4 border-t border-[#e2e8f0] bg-slate-50/50">
        <button
          onClick={logout}
          className="flex items-center gap-3 w-full px-4 py-3 rounded-lg text-sm font-semibold text-rose-600 hover:bg-rose-50 transition-colors text-left cursor-pointer"
        >
